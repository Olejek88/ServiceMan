...
import ru.shtrm.serviceman.data.ObjectContragent;
...
public class ContragentLocalDataSource implements ContragentDataSource {
    ...
    public Contragent getContragentByObject(String uuid) {
        Realm realm = Realm.getDefaultInstance();
        ObjectContragent objectContragent = realm.where(ObjectContragent.class).
                equalTo("object.uuid", uuid).findFirst();
        if (objectContragent != null) {
            Contragent contragent = realm.where(Contragent.class).
                    equalTo("uuid", objectContragent.getContragent().getUuid()).findFirst();
            if (contragent != null) {
                contragent = realm.copyFromRealm(contragent);
                realm.close();
                return contragent;
